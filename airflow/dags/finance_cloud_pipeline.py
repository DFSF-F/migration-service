...
RAW_EXPORT_DIR = Path("/opt/airflow/project/scripts/cloud_export/finance")
BQ_SQL_DIR = Path("/opt/airflow/project/bigquery")
GCS_PREFIX = "finance/raw"
...
def export_finance_raw_from_greenplum_to_parquet() -> None:
    RAW_EXPORT_DIR.mkdir(parents=True, exist_ok=True)

    hook = PostgresHook(postgres_conn_id="greenplum_dwh")
    engine = hook.get_sqlalchemy_engine()

    for source_table in FINANCE_RAW_TABLES:
...
def upload_finance_raw_to_gcs() -> None:
...
def load_finance_raw_to_bigquery() -> None:
    bucket_name = _required_env("GCS_STAGING_BUCKET")
    location = _required_env("GCP_REGION")
    ...
    for source_table in FINANCE_RAW_TABLES:
        ...
        print(f"Start loading {source_uri} -> finance_raw.{table_name} ({location})")
        ...
        print(
            f"Loaded {source_uri} -> finance_raw.{table_name}. "
            f"State={payload.get('status', {}).get('state')}"
        )


def validate_finance_raw_counts() -> None:
    project_id = _required_env("GCP_PROJECT_ID")
    location = _required_env("GCP_REGION")
    ...
    for source_table in FINANCE_RAW_TABLES:
        ...
        query = f"""
        select count(*) as cnt
        from `{project_id}.finance_raw.{table_name}`
        """
...
def build_finance_dds() -> None:
    location = _required_env("GCP_REGION")
    project_id = _required_env("GCP_PROJECT_ID")

    sql = _read_sql_file(BQ_SQL_DIR / "finance_dds.sql").replace("{{PROJECT_ID}}", project_id)
    result = _run_bq_query(sql, location=location)

    print(f"finance_dds.sql executed. jobComplete={result.get('jobComplete')}")


def build_finance_dm() -> None:
    location = _required_env("GCP_REGION")
    project_id = _required_env("GCP_PROJECT_ID")

    sql = _read_sql_file(BQ_SQL_DIR / "finance_dm.sql").replace("{{PROJECT_ID}}", project_id)
    result = _run_bq_query(sql, location=location)

    print(f"finance_dm.sql executed. jobComplete={result.get('jobComplete')}")


with DAG(
    dag_id="finance_cloud_pipeline",
    start_date=datetime(2024, 1, 1),
    schedule=None,
    catchup=False,
    tags=["gcp", "bigquery", "gcs", "finance", "migration", "parquet"],
) as dag:
    ...
    export_raw = PythonOperator(
        task_id="export_finance_raw_from_greenplum_to_parquet",
        python_callable=export_finance_raw_from_greenplum_to_parquet,
    )

    upload_raw = PythonOperator(
        task_id="upload_finance_raw_to_gcs",
        python_callable=upload_finance_raw_to_gcs,
    )
    ...
    load_raw = PythonOperator(
        task_id="load_finance_raw_to_bigquery",
        python_callable=load_finance_raw_to_bigquery,
    )

    validate_raw = PythonOperator(
        task_id="validate_finance_raw_counts",
        python_callable=validate_finance_raw_counts,
    )

    build_dds = PythonOperator(
        task_id="build_finance_dds",
        python_callable=build_finance_dds,
    )

    build_dm = PythonOperator(
        task_id="build_finance_dm",
        python_callable=build_finance_dm,
    )
