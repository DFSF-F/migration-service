...
def build_access_dds() -> None:
    location = _required_env("GCP_REGION")
    project_id = _required_env("GCP_PROJECT_ID")

    sql = _read_sql_file(BQ_SQL_DIR / "access_dds.sql").replace("{{PROJECT_ID}}", project_id)
    result = _run_bq_query(sql, location=location)

    print(f"access_dds.sql executed. jobComplete={result.get('jobComplete')}")


def build_access_dm() -> None:
    location = _required_env("GCP_REGION")
    project_id = _required_env("GCP_PROJECT_ID")

    sql = _read_sql_file(BQ_SQL_DIR / "access_dm.sql").replace("{{PROJECT_ID}}", project_id)
    result = _run_bq_query(sql, location=location)

    print(f"access_dm.sql executed. jobComplete={result.get('jobComplete')}")
...
with DAG(
    ...
    schedule=None,
    catchup=False,
    tags=["gcp", "bigquery", "gcs", "access", "migration", "parquet"],
) as dag:
    ...
    build_dds = PythonOperator(
        task_id="build_access_dds",
        python_callable=build_access_dds,
    )

    build_dm = PythonOperator(
        task_id="build_access_dm",
        python_callable=build_access_dm,
    )
