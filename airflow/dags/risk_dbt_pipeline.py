...
with DAG(
    ...
    dbt_run_risk = DockerOperator(
        task_id="dbt_run_risk",
        image=DBT_IMAGE,
        api_version="auto",
        auto_remove="success",
        command=f"bash -c 'cd {DBT_PROJECT_DIR} && dbt clean && dbt run --select path:models/risk --threads 1 --no-partial-parse'",
        docker_url="unix://var/run/docker.sock",
        network_mode="migration-service_default",
        mount_tmp_dir=False,
        mounts=DBT_MOUNTS,
